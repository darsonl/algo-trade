...
- [x] **TEST-10**: Config `validate()` tested — missing required env vars raise `ValueError` (both `ANALYST_PROVIDER` branches); a fully-valid config passes; `USE_LIMIT_BUY` maps correctly (unset → `False` default, `=false` → `False`, `=true` → `True`)
