...
| `analyst/news.py` | 5 headlines per ticker: Alpha Vantage NEWS_SENTIMENT when a key is set (headlines carry a sentiment label), yfinance otherwise and on any failure. `redact_secret` + `AlphaVantageUnavailable` + the day breaker |
...
- **A provider's error message is not safe to log — Alpha Vantage quotes your API key back at you** (2026-09-15). The free tier's refusal body is literally `We have detected your API key as <KEY> and our standard API rate limit is 25 requests per day`, and `news.py` wrapped it in a `RuntimeError` and logged it verbatim. Measured on the live log: **43 occurrences** across two sessions. `logs/` is gitignored and the key never reached any ref, so this was plaintext-on-disk rather than a public disclosure — but this repo is public, and one pasted log tail closes that gap.
  ...
  **Known, unfixed: headline provenance correlates with S&P rank.** The budget is consumed in scan order, so on 2026-09-15 positions 0–24 got sentiment-scored headlines and 25–40 got plain ones — a clean step function, verified. The analyst therefore sees systematically richer input for higher-ranked candidates. The outcome split (AV 5/25 recommended vs yf 1/16) is far too small to mean anything (Fisher ≈ 0.38); the problem is that the confound is **guaranteed by construction rather than random**, so it will not average out across sessions. It is *recoverable* from existing data — Alpha Vantage headlines are self-identifying by their `[Label, score]` suffix in `headlines_json` — so this is a cohort-splitting caveat, not data loss. Recording provenance explicitly (the `reference_price_source` pattern) is the proper fix.
...
1563 tests as of 2026-09-15. Run with `.venv/Scripts/python.exe -m pytest -q` (~35s). Without a `.env` (a fresh worktree), 7 scan tests fail on `openai.OpenAIError` unless the four dummy analyst keys from `.github/workflows/ci.yml` are exported — an environment gap, not a regression.
...
- `test_analyst_news.py` — the Alpha Vantage path, which had **no** coverage at all until the key leak was found: redaction at the raise, a quota refusal not retried, a transient failure still retried, the day breaker and its roll-over (19 tests)
